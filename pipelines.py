...
# Lets first make sure CUDA is available. This is just to check if we
# have set things up properly and we have access to a GPU.
print(f"CUDA Available: {torch.cuda.is_available()}")

if not torch.cuda.is_available():
    raise ValueError(
            "CUDA is not available, something is wrong. Make sure "
            "you are running this code on a worker node if using "
            "adroit. You can run this script as follows:\n\n "
            "\tsalloc -t 0:1:00 --ntasks=1 --gres=gpu:1 python pipelines.py"
            )
...
print("Sentitment Classification:\n")
...
outputs = zero_shot_classifier(text, candidate_labels=classes)
